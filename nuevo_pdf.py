...
Original file is located at
    https://colab.research.google.com/drive/1Jl7UgoS8PyPg7pvWVUMEmRieezH1fIhe
"""

# -*- coding: utf-8 -*-
"""APP_modulo1_V3_mejorado.ipynb"""
...
def generar_pdf(datos_personales, resultados, comparativo, global_diag, imagenes):
    ...
    pdf.cell(35, 8, "Varianza", 1)
    pdf.cell(30, 8, "RMS", 1)
    pdf.cell(50, 8, "Amplitud Máxima (cm)", 1)
    pdf.ln()
    pdf.set_font("Arial", '', 9)

    for _, row in resumen.iterrows():
        pdf.cell(40, 8, str(row["Test"]), 1)
        pdf.cell(35, 8, f"{row['Freq_Dom']:.2f}", 1)
        pdf.cell(35, 8, f"{row['Varianza']:.2f}", 1)
        pdf.cell(30, 8, f"{row['RMS']:.2f}", 1)
        pdf.cell(50, 8, f"{row['Max_Despl']:.2f}", 1)
        pdf.ln()
    ...
    if global_diag == "Posible Parkinson":
        texto_diag = "El análisis muestra una frecuencia característica de 4-6 Hz con amplitudes significativas, lo cual podría corresponder a un temblor de tipo Parkinsoniano."
    elif global_diag == "Posible Temblor Esencial":
        texto_diag = "El análisis presenta frecuencias de 6-12 Hz con amplitudes moderadas, sugiriendo un posible Temblor Esencial."
    else:
        texto_diag = "El análisis no muestra patrones compatibles con temblores patológicos claros."
